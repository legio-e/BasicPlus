/*
 * aot_XmodNat.c — AUTOGENERADO por AotCEmitter (H3 #157).
 * NO EDITAR A MANO. Regenerar compilando XmodNat.bp con --aot.
 *
...
static int32_t aot_XmodNat_compute(struct bpvm* vm, int32_t x);
...
static void thunk_XmodNat_compute(struct bpvm* vm,
    ...
    int32_t r = aot_XmodNat_compute(vm, a0);
...
void aot_XmodNat_register(struct bpvm* vm) {
    bpvm_aot_register_by_name(vm, "XmodNat.compute", thunk_XmodNat_compute);
}
