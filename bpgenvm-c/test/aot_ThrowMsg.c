/*
 * aot_ThrowMsg.c — AUTOGENERADO por AotCEmitter (H3 #157).
 * NO EDITAR A MANO. Regenerar compilando ThrowMsg.bp con --aot.
 *
...
static int32_t aot_ThrowMsg_checkRange(struct bpvm* vm, int32_t n);
...
static int32_t aot_ThrowMsg_checkRange(struct bpvm* vm, int32_t n) {
    ...
    (void) n;
    if ((n < 0)) {
        vm->aot_helpers->throw_str(vm, vm->aot_helpers->string_concat(vm, vm->aot_helpers->string_from_cstr(vm, "valor negativo: ", 16), vm->aot_helpers->int_to_string(vm, n)));
    }
    return (n * 2);
}

static void thunk_ThrowMsg_checkRange(struct bpvm* vm,
                              uint32_t* sp_p,
                              uint32_t* bp_p) {
    (void) bp_p;
    ...
    int32_t r = aot_ThrowMsg_checkRange(vm, a0);
...
void aot_ThrowMsg_register(struct bpvm* vm) {
    bpvm_aot_register_by_name(vm, "ThrowMsg.checkRange", thunk_ThrowMsg_checkRange);
}
