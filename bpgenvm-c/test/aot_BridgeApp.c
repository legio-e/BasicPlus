/*
 * aot_BridgeApp.c — AUTOGENERADO por AotCEmitter (H3 #157).
 * NO EDITAR A MANO. Regenerar compilando BridgeApp.bp con --aot.
 *
...
static int32_t aot_BridgeApp_compute(struct bpvm* vm, int32_t n) {
    ...
    (void) n;
    return (vm->aot_helpers->call_bp_i32(vm, vm->aot_helpers->find_function(vm, "BridgeLib.triple"), (int32_t[]){ n }, 1, 0u, 0) + 1);
}
