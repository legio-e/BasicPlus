/*
 * aot_NativeBridge.c — AUTOGENERADO por AotCEmitter (H3 #157).
 * NO EDITAR A MANO. Regenerar compilando NativeBridge.bp con --aot.
 *
...
static int32_t aot_NativeBridge_compute(struct bpvm* vm, int32_t n) {
    ...
    (void) n;
    return (vm->aot_helpers->call_bp_i32(vm, vm->aot_helpers->find_function(vm, "NativeBridge.helper"), (int32_t[]){ n }, 1, 0u, 0) + 1);
}
