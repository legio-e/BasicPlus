/*
 * aot_CompressNat.c — AUTOGENERADO por AotCEmitter (H3 #157).
 * NO EDITAR A MANO. Regenerar compilando CompressNat.bp con --aot.
 *
...
static int32_t aot_CompressNat_decompress(struct bpvm* vm, int32_t src, int32_t srcLen, int32_t dst, int32_t dstCap);
...
static void thunk_CompressNat_decompress(struct bpvm* vm,
                              uint32_t* sp_p,
                              uint32_t* bp_p) {
    (void) bp_p;
    ...
    int32_t a2 = (int32_t) H->read_ref(mem + sp - 8); sp -= 8;  /* ref: 8B */
    int32_t a1 = H->read_i32_be(mem + sp - 4); sp -= 4;
    int32_t a0 = (int32_t) H->read_ref(mem + sp - 8); sp -= 8;  /* ref: 8B */
    int32_t r = aot_CompressNat_decompress(vm, a0, a1, a2, a3);
...
void aot_CompressNat_register(struct bpvm* vm) {
    bpvm_aot_register_by_name(vm, "CompressNat.decompress", thunk_CompressNat_decompress);
}
