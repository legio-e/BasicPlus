/*
 * aot_Bench.c — AUTOGENERADO por AotCEmitter (H3 #157). NO EDITAR A MANO.
 * Regenerar compilando samples/benchmarks/Bench.bp con el pipeline AOT.
 *
 * H4 AOT RISC-V (Hito 1): es EL MISMO fichero portable que pico/aot_Bench.c
 * (ARM). Aquí lo compila el toolchain del P4 (riscv32-esp-elf-gcc) → prueba de
 * que el C del AOT es universal: "native" = solo recompilar con otro toolchain,
 * sin codegen por-arquitectura (principio de Eduardo). Sin refs al heap (fib
 * leaf) → el #302 paso 3 (raíces GC del nativo) no aplica todavía.
 */
