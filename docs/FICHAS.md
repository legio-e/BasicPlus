...
buena, y un síntoma que apunta a otro sitio. **Medido otra vez el 11-sep en el P4 (`C1`)**: con
`/lib/Gui.mod` fresco, `GuiShot` enlazaba contra la `Gui` del `Stdlib.pack` del 20-ago — el overlay de
packs de la fachada (`fs_facade.c:fallback_claims`) contesta al `stat` de **`/app/Gui.mod`**, el primer
candidato del resolutor, y `/lib` no llega a mirarse. O sea, el pack sombrea a `/lib` **disfrazado de
`/app`**, y el aviso «el FS eclipsa al del pack» sale al revés. Se resolvió borrando los packs de la placa. Y el linker **sí** grita en el desfase de métodos
...
#### 📸 `#475` — CAPTURA DE PANTALLA EN EL MICRO: el testigo de las pruebas gráficas — ✅ **CONSTRUIDA y VERIFICADA EN LAS CUATRO PANTALLAS el 11-sep** (`0791bb3e`, `d9c6e0db`)
...
**El P4, la misma noche** (`idf.py -p COM14 flash` entra solo por el CP210x): **1024×600, 25 bloques,
8 885 B (138×), `GET` en 820 ms**; y con `Gui.setRotation(90)` la captura es la **lógica** 600×1024 con
`rot=1` en la cabecera, contenido correcto (la rotación por software del flush queda por debajo del gancho,
como decía la spec). Antes de eso, un tropiezo que es de `#493`: el `GuiShot` enlazaba contra una **`Gui`
vieja** aunque `/lib/Gui.mod` fuera la fresca — la **zona de packs** del P4 (un `Stdlib.pack` del 20-ago)
contesta al `stat` de `/app/Gui.mod` por el overlay de la fachada (`fs_facade.c:fallback_claims`), que es el
primer candidato del resolutor, así que `/lib` nunca llega a mirarse. Eduardo borró los packs y enlazó.

⏭️ **Queda**, si `T1`/`D1` lo piden: un visor de `.shot` en el IDE (hoy `shot2png.py`). De aquí salió
**`#495`** (V7).
