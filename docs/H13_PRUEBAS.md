...
- ✅ **`samples/imageproject`** (módulo `ImageDemo`) — imagen desde `resources/`.
  Funciona **porque es un PROYECTO** y el IDE sube el `resources/` con el Run
  (#260) — explicación de Eduardo, y es la que confirma el hallazgo **25** en vez
  de contradecirlo: el roto es el **sample suelto** `samples/GuiImageDemo.bp`,
  que ni es proyecto ni lleva su `.png`. ⚠️ **Los dos nombres se parecen
...
- ⚠️→✅ **`ChartDemo`** — no pintaba en placa, pero **no era el widget**: al sample
  le faltaba `Gui.run()` (**hallazgo 20**). **Arreglado el mismo día** (es un `.bp`,
  cero firmware) y **el widget verificado pintando en el host**. En placa se
  reprobará cuando se reselle el ZIP.
...
**🔑 EL DISCRIMINADOR (Eduardo, 6-ago) — y es lo que cierra el asunto para V4:**

> *«Ayer las pruebas con las STM32, el stop funcionó perfectamente, en cambio con
> las P4 iba a veces. Hay algo diferente entre las dos pero lo mismo, a
> investigar en V5.»*

Esto **no es un matiz, es el control que faltaba**, y hay que leerlo al revés de
como apetece: no dice dónde está el fallo, dice **dónde NO está**.

- **Descarta el lado del PC.** El drenaje del `connect` y el handshake son el
  **mismo código Java** para las dos (`BpvmClient.connectSerial` →
  `doHandshake`, sin ninguna rama por familia). Si la causa fuera el drenaje de
  300 ms fijos —que es a donde yo iba—, la Nucleo fallaría igual. No falla.
- **Descarta también el KILL**, que ya estaba descartado por otra vía: es núcleo
  portable, idéntico en las tres familias.
- **Lo que queda es lo que SÍ cambia entre esas dos placas**: el transporte
  físico y quién lo bufferiza. La STM32 habla por el **VCP del ST-LINK** (UART
  real, 115200, ritmo constante); el P4 por su propio puente. Y con él cambia
  cuánto texto puede quedar en vuelo cuando el programa muere de golpe.

Lo anoto porque es exactamente el patrón que me ha mordido dos veces esta
semana: **cuando un fallo aparece en una placa y no en otra, la pregunta no es
«qué falla» sino «qué es distinto»** — y aquí lo distinto no es el software que
yo estaba mirando. Iba a tocar el drenaje del `connect` del IDE a ciegas, con un
mecanismo plausible y **cero repro**; el dato de Eduardo dice que me habría
equivocado de fichero. Se para ahí: **→ V5, tarea #379**, y con el discriminador
ya en la mano, que es la mitad del trabajo.

*(Y encaja con el criterio de siempre: [[debug-acotar-antes-de-tocar]]. Lo que
faltaba no era una idea mejor, era un caso que separase las dos mitades. Lo
aportó el banco de pruebas, no el análisis.)*
...
| 12 | a1 Pico | En **cada** RUN: `MDN: RECHAZADO — ABI 1, esta VM habla 2`. El `Bench.mdn` embebido (`pico/embedded_bench_mdn.c`, 26-may) es anterior a que los helpers AOT pasaran a v2 (#302 paso 2). Inofensivo —el gate lo rechaza y se queda el linked-in— pero asusta en cada ejecución. **Tercer artefacto generado embebido que se queda atrás**, tras el Hello .mod en 3 familias | ✅ **RETIRADO (6-ago), y era MÁS de lo anotado.** Al ir a quitarlo aparecieron **tres** andamios de la misma tarea (#157/#158/#159), no uno: (1) `embedded_bench_mdn.c` —el blob ABI 1 que provocaba el mensaje—; (2) `aot_Bench.c`, el AOT de `Bench.bp` **compilado dentro de la imagen**, precargado antes que nada; y (3) **`bench.c`, que corría un `fib(28)` en C EN CADA ARRANQUE** — ahí estaban los ~105 ms del boot que ya habíamos medido y atribuido al `.mdn`. Los tres servían para decidir *«si el ratio C:BP < 2× aparcamos AOT»*, decisión tomada hace un año y con el AOT publicado a 113×/116×. Fuera los tres + su alta en CMake; **queda el mecanismo bueno intacto**, que es el stage 3 (escaneo de `.mdn` en el FS, `repl_v1.c`) — el que usa el IDE y el que se probó en las tandas. `aot_funcs.c` sobrevive sólo como hook de trazas del cargador. Imagen reconstruida en verde |
| 11 | a1 Pico | Una imagen nueva **no actualiza lo que ya está en el FS**: el preinstalado hace `if (fs_exists(...)) continue`, así que el `/app/Hello.mod` v5 sobrevivió al reflasheo. Coherente —no queremos pisar ficheros del usuario— pero un blob preinstalado que cambia de versión se queda rancio para siempre en placas con FS previo | 🟢 **NO SE TOCA EN V4 → V5.** No es peligroso: el gate de ABI caza el fichero rancio. Lo correcto —reemplazar el preinstalado cuando el de la imagen es **distinto** del que hay— es un cambio de firmware que no cabe en el freeze. Rodeo documentado: formatear el FS al cambiar de imagen |
...
| 19 | b1 S3 | `BoardTest` imprime **`variant=B`** en un ESP32-S3. Los otros dos números son correctos (`gpioCount=45`, `GPIO_COUNT=45`), lo que lo empeora: **el dato falso viaja envuelto en dos verdaderos**. Sale de `bpstdlib/Pico.bp:92`, que **no es intrínseco**: cinco líneas de BP puro, `if gpioCount() >= 40 then "B" else "A"` — el umbral separa RP2350A (30 GPIO) de RP2350B (48), y los 45 del S3 caen del lado del 48. Visto por Eduardo en el log de la tanda 7 | 🟢 **NO SE TOCA — reencuadre de Eduardo**: `Pico.bp` es **la librería de la familia RP2350**, no una capa común, y `variant()` hace lo que documenta. Lo que falta es la librería de placa GENÉRICA → tarea **#371** de V5 |
| 20 | — (paquete) **+ c2** | **El widget `Chart` es lo NUEVO de V4 en gráficos (H7, #317) y se publica sin demo — y AL PROBARLO, la demo no pinta.** El `ChartDemo.bp` existe y está escrito —caso realista, ventana deslizante de muestras de sensor con dos series— pero vive en `bpgenvm-c/samples/`, que es la carpeta de desarrollo de la VM-C y **no viaja al ZIP** (el copiado es `git ls-files 'samples/*.bp'`). Resultado: la stdlib expone `Gui.Chart` con sus seis intrínsecos y **no hay un solo `.bp` publicado que lo use** — verificado con `grep -rln Chart --include=*.bp`, que en todo el árbol sólo da `bpstdlib/Gui.bp` y ese fichero fuera de sitio. Encontrado al montar la escalera de GUI del P4 | ✅✅ **CERRADO Y VERIFICADO (6-ago)**: `ChartDemo.bp` movido a `samples/` (viaja en el ZIP) y **pinta en el micro simulado desde la instalación limpia** — puerta final 2/3. Ver la sección de cierre |
🔎 **PROBADO POR FIN (5-ago, c2) — y el sample está RANCIO, el widget NO está roto.** Eduardo lo corrió en la Discovery: *«no muestra nada»*, y decidió —bien— **no bloquear**: *«como esto es nuevo de V4 déjalo pasar, apunta revisar para V5»*. Pero antes de dejarlo como «revisar» a secas hacía falta separar **sample rancio** de **widget roto**, porque son dos gravedades distintas: si el widget estuviera mal, V4 publicaría `Gui.Chart` con sus seis intrínsecos **sin funcionar**, y eso importa aunque la demo no viaje. **Separado en el host** (con el simulador ya reconstruido con LVGL, hallazgo 37): la salida es `series / 0 / 1 / ok` y el volcado enseña **`chart [200x120]` colgando del screen**. O sea que **el widget se crea y la API responde**. 🎯 **La causa es de una línea: `ChartDemo.bp` NO LLAMA A `Gui.run()`.** Termina en `print Gui.__guiDumpTree()` y sale (`:34-35`). Nació en H7 como prueba **del MODELO**, antes de que el GUI fuera por eventos, y nadie lo actualizó: **nunca entra en el bucle de la interfaz, así que no hay nada que pintar**. En placa el síntoma es exactamente el que describe Eduardo. ⇒ **La nota de V5 deja de ser vaga**: no es *«investigar el Chart»* sino **«añadir `Gui.run()` al sample, moverlo al ZIP y volver a probarlo»**. ✅ **ARREGLADO Y CON VEREDICTO (5-ago).** Criterio de Eduardo: *«podemos arreglar la demo, eso no es problema; lo importante es que no haya que rehacer las imágenes de los micros»* — y es exactamente el caso: **es un `.bp`, cero firmware**. Añadido `Gui.run()` al final de `ChartDemo.bp` con el comentario de por qué. **Verificado pintando de verdad**: ejecutado en el host con `BPVM_GUI_SHOT_MS` y **captura mirada**, no deducida — se ven **las dos series** (roja ascendente `i*10`, azul descendente `100-i*5`), la rejilla y los puntos. ⇒ **`Gui.Chart` deja de estar sin veredicto: FUNCIONA.** Lo que quedaba de V4 en gráficos ya no se publica a ciegas. ⏳ Queda sólo la mitad de empaquetado: **moverlo a `samples/`** para que viaje en el ZIP (esto y el 35 comparten arreglo) |
| 38 | c2 STM32 | ⚠️ **`Gui.setRotation` NO ROTA en LTDC: el evento se dispara, el modelo gira y el panel no.** Visto por Eduardo en `GuiRotDemo` sobre la Discovery. **No es un misterio: está escrito en el código.** `stm32/port/gui_display_ltdc.c:153` es un **no-op declarado**: *«el LTDC escanea un framebuffer fijo y este flush NO gira aún (se haría como en el P4-ws: `lv_draw_sw_rotate` en el flush)»*, y avisa una vez con `[gui] setRotation: no soportado en este display (LTDC) todavia`. Lo que sí ocurre es la **mitad lógica**: `src/gui.c:288` intercambia ancho/alto del screen del modelo —para mantener la paridad con miVM— así que el programa *cree* que ha girado. De ahí el síntoma exacto que describe Eduardo | 🟡 **AL REGISTRO → V5. Es hueco de ALCANCE, no defecto**, y de la familia del hallazgo 21 (NeoPixel sin driver) **pero en su versión buena**: aquí la fachada **avisa** en vez de contestar éxito en silencio. La rotación se implementó para el P4-ws y no para LTDC; encaminar el verbo no es implementarlo ⇒ es el patrón *«portar a otra familia = adaptador COMPLETO»*. ⛔ **Consecuencia para la fila**: `GuiRotDemo` **NO PUNTÚA** en la c2, igual que `NeoTest` no puntúa donde no hay driver. ⏳ Pendiente de confirmar con Eduardo si **vio el aviso en consola**: si sale, el alcance está anunciado y se queda en V5; si NO sale, sube de categoría porque sería un instrumento mudo |
| 37 | — (paquete) | ✅✅ **CERRADO Y VERIFICADO (6-ago)**: `bpvm-sim.exe` reconstruido con LVGL (2014 símbolos + SDL2) y **pintando desde la instalación limpia** — puerta final 2/3 |
🟡 **AL REGISTRO → al lote del ZIP.** Es la trampa conocida de la casa: **`make` sin `LVGL=1` envenena los `.o`** y el siguiente `make LVGL=1` **no los rehace**, así que el arreglo exige **limpiar antes de reconstruir**. No toca firmware ⇒ no choca con la regla del 5-ago; entra con el 20, el 25, el 35 y el 36. **Alcance honesto**: no rompe ninguna placa —H10 sirve para desarrollar sin hardware— pero deja **medio hito entregado**: el micro simulado ejecuta, habla por el wire y tiene FS, y **no tiene pantalla**, que es justo lo que uno espera de un simulador de una placa con panel. Y el IDE ofrece configurar un tamaño de pantalla que no hace nada. ⚠️ **Efecto colateral en las pruebas**: cualquier verificación gráfica hecha en el simulador **no vale** — no porque mienta, sino porque no pinta. Es lo que impidió comprobar el hallazgo 36 por esta vía |
| 36 | — (paquete) | ✅✅ **CERRADO Y VERIFICADO (6-ago)**: la fuente existía —la tenía Eduardo, de su demo de MicroPython— y **nunca estuvo en git**; la limpieza de V3 se la llevó. Va ya en `samples/` y **se ha visto cargando con captura**. Queda a V5 lo de fondo: `loadFont` **no puede decir que ha fallado** |
🔴 **Y encima el sample no se entera, que es la mitad que más molesta.** `:29` hace `cargada.setFont(fid)` **sin mirar `fid`**, así que con la fuente ausente el label **se pinta igual con la de por defecto** y la demo *parece correcta*. Es exactamente la trampa del hallazgo 21 —**un instrumento que se declara verde sin haber hecho nada**— y explica que en el P4 quedara anotado como bueno: se vio *una* fuente pintada, no *la* fuente cargada. ⚠️ **Eso obliga a rebajar el resultado del P4**: `FontLoadDemo` en la b2 **no demuestra que `loadFont` funcione**; sólo que no revienta. |
🟡 **AL REGISTRO → al lote del ZIP.** Es el **cuarto** de la misma familia (20 `ChartDemo`, 25 `GuiImageDemo`, 35 benchmarks/AOT): *un sample publicado que no se puede ejecutar tal como llega*. No toca firmware. Arreglo: **generar el `.bin` con `lv_font_conv`, meterlo en el ZIP** y —esto es lo importante— **que el sample compruebe `fid`** y diga *«no se pudo cargar la fuente»* en vez de pintar como si nada. Mientras no esté, **`FontLoadDemo` no puntúa**: ni aquí ni en la b2 |
| 35 | — (paquete) | 🔴 **El AOT —el hito grande de V4— se publica SIN UN SOLO EJEMPLO.** Visto por Eduardo al correr el bench en la c1: *«el Bench no está en las samples del zip pero sí donde siempre»*. Verificado: `scripts/montar-zip.sh:114` hace `case "${f#samples/}" in */*) continue ;; esac`, o sea **descarta todo lo que viva en un subdirectorio** de `samples/`, y luego copia a mano sólo tres proyectos (`sampleproject`, `formdemo`, `imageproject`) más `errores/`. Resultado: **`samples/benchmarks/` entero —8 ficheros— se queda fuera**, y con él `Bench.bp`, que es el que enseña `native` + el gemelo interpretado para medir la ganancia en la misma placa. **Comprobado que el agujero es total**: `grep` de `native function` sobre los samples de la RAÍZ —los únicos que viajan— da **cero**; los cinco que lo usan (`Bench` `intbench` `floatbench` `arraybench` `fibobench` `sortbench`, más `aottest/`) están todos en subcarpetas. O sea que **un usuario que instala V4 lee `native` en el manual y no tiene un solo `.bp` que ejecutar para verlo** | ✅ **CERRADO (6-ago) — y el arreglo es de MECANISMO**: listas explícitas `PUBLICAR_DIRS`/`INTERNAS_DIRS` **más un guardián que impide montar el paquete** si aparece una subcarpeta de `samples/` sin decidir. Entran `benchmarks/` (con `Bench.bp`) y `aottest/`, y también `external/` y `plugins/`, de las que dependían tres samples publicados |
| 34 | c1 y c2 (**y todas**) | ✅✅ **CAZADO Y ARREGLADO — `paralleltest_sugar` muere con `exit 11 (RUNTIME_ERROR)` en la Nucleo — y sin decir por qué.** El programa arranca, imprime `[main] arrancando 2 ramas + default` y `[default] localDefault = 12345`, y **muere antes de que t1 y t2 impriman su resultado**: o sea que **el `default` corre y las dos ramas `case` no**. Lo que ya está comprobado:
...
🔴🔴 **REABIERTO (5-ago, tarde) — Eduardo lo corrió «por si acaso» en la c2 y FALLA IGUAL.** Misma firma exacta: imprime `[main] arrancando…` y `[default] localDefault = 12345`, y muere con `exit 11` **antes de que las dos ramas `case` den su resultado**. **Otra placa, otro chip, otro binario** — y el mismo sample.

**Y eso obliga a deshacer mi conclusión de esta mañana.** Yo lo cerré como *«es el 24 otra vez»* porque un reset lo curaba, o sea **estado acumulado**. Pero había un dato que no pesé bastante: en la tanda 4 de la c1 corrieron **11 samples de threads y sólo falló ÉSTE**. Si además reaparece en una segunda placa, la explicación «se degrada con el uso» deja de ser suficiente: **este sample tiene algo propio**.

🔍 **Y lo que tiene propio está identificado**: es el **único** de los 11 que usa el **azúcar `parallel/case/default/endpar`**, o sea **subclases ANÓNIMAS de `Thread` que fabrica el compilador**. Su gemelo `paralleltest` hace lo mismo con una clase **declarada por el usuario** (`Worker extends Thread`) y **pasó en la c1** — 56.781 ms, verde. Mismo tamaño de pila (2048) en los dos.

⏳ **PENDIENTE DE UN EXPERIMENTO DE DOS PASOS, y es barato** (misma placa, mismo momento, se cambia **una sola cosa**):
1. **`paralleltest`** (el gemelo SIN azúcar) **ahora, sin resetear**. Si pasa ⇒ no es estado acumulado, **es el camino del azúcar**.
2. Luego **reset** y `paralleltest_sugar` otra vez. Si entonces va, la degradación **también** influye, y son dos cosas superpuestas.

✅✅ **CERRADO — es un BUG DEL COMPILADOR, y el arreglo es de una línea.**

**Cómo se cazó, y el mérito es del método, no de la suerte.** Eduardo repasó **la secuencia** de la c2 y vio lo que yo no: allí **no había corrido ni un test de threads** —tanda 1, `MemInfo`, tanda 3 y ya— así que *«se degrada con los threads»* quedaba descartado y **`MemInfo` se convertía en el sospechoso**. De ahí salió la receta determinista, verificada en placa:

    reset + MemInfo + paralleltest_sugar  →  error 11
    reset +           paralleltest_sugar  →  OK

Con eso monté **el test fusionado** que él mismo había propuesto por la mañana —agotar el heap y a continuación el `parallel/case`, **en un solo programa**— y **reprodujo EN EL HOST, en UNA ejecución**. Dos consecuencias inmediatas: no hace falta placa, y **el límite del RUN no pinta nada** (era *por operación*, no *por RUN* — la pregunta que decidía la gravedad).

**Y falla en LAS DOS VMs** ⇒ el defecto está en el **bytecode que emite el compilador**, no en un runtime. La VM-C además lo nombra sola:
...
🔧 **La causa, en `MivmEmitter.emitParallel`**: la variable visible del usuario se declaraba con **`declareLocal(br.varName)` = 4 BYTES**, y ahí se guarda un **handle de 8**. El `emitSetLocal` truncaba la referencia al thread y el `emitGetLocal` de `start()`/`join()` trabajaba con media referencia. **Es el sitio que se saltó la pasada H1.8** —la que centralizó el ancho de ref— y por eso su gemelo `paralleltest` no falla: el camino normal de `var x: Thread := …` ya declaraba **por tipo**. Mismo animal que **#369**, **#281** y **#293**.

**Por qué no mordía antes, que es lo que lo hacía invisible**: los 32 bits que se pierden son los de la **generación**, y mientras nadie recicla un slot todas valen **0** — o sea que el handle truncado *funciona por casualidad*. `MemInfo` recicla slots (reserva hasta OOM y suelta), las generaciones suben a 1, y el truncamiento se ve. **El bug estaba SIEMPRE ahí**; lo que hacía falta era un programa que reciclara objetos antes de usar el azúcar. Eso valida la advertencia de Eduardo de esta mañana: *«inofensivo en pruebas, mortal en explotación»*.

**Arreglo** (`declareLocal` → `declareLocalRef`, más el gemelo con tag): verificado en **las dos VMs** con el repro, **9/9 samples de threads sin errores** en las dos, y **287 samples de raíz compilados** con sólo los **2 fallos preexistentes y ya conocidos** (`appv1lsp`/`appv2`, la pasada interfaz-only que tira firmas). Repro guardado en `diag/parsugar-uaf/`.

⚠️ **Y una metida de pata mía por el camino, que vale la pena dejar escrita**: aproveché para darle a `declareLocalRef` el guarda de scope de sus hermanos *«por uniformidad»* — y **rompió `modpropsync`** (NPE en el emisor). Revertido: **se cambia UNA cosa**. La rareza queda anotada en el código, no arreglada de paso.

💰 **Coste de publicación: BAJO.** Es frontend ⇒ **`.mod` nuevos + resellado de ZIP**, y **NINGÚN reflasheo**: comprobado que **ni `bpstdlib/` ni `bpdevices/` usan `parallel`**, así que los blobs embebidos en los 5 firmwares no cambian. No choca con la regla del 5-ago |
...
| 32 | c1 STM32 | 🟢 **NO SE TOCA EN V4 — decisión de Eduardo (6-ago)**: *«es informativo, no afecta para el resto de funcionalidades»*. Y **el arreglo evidente era el malo**: yo iba a unificar los dos números a mano, y Eduardo puso el dedo en por qué no vale — *«STM fabrica la misma CPU con diferentes encapsulados, diferentes pins pero también diferentes periféricos; si queremos una imagen única para la familia tenemos que soportar el más grande y el más pequeño»*. O sea que **el número correcto no existe en tiempo de compilación**: hay que preguntárselo a la placa. Eso es la tarea **#378** de V5 (capa HAL BP de capacidades, tirando de lo que dé la HAL del fabricante). Aviso técnico que va con ella: en el STM32 los pines se numeran `puerto*16+bit` y son **dispersos**, así que *ni 114 ni 128* sirven para recorrerlos — lo que hace falta no es un contador mejor, es **poder preguntar si un pin existe**. La divergencia queda anotada en el código, junto al string del INFO, para que nadie la «arregle» por el camino corto | 🟡 **AL REGISTRO → al lote.** Es la doctrina de la casa incumplida —**una función, una fuente**— la misma de #299 (layout de clase) y #315 (slots de vtable). El arreglo es que el INFO pregunte al backend en vez de llevar el número horneado, como ya hacen ADC y PWM: **una familia, una imagen**. ⚠️ **Y antes hay que DECIDIR cuál es el número bueno**, que no es obvio: 114 es lo que el usuario puede pinchar, 128 es lo que el driver direcciona. Yo publicaría **114** —el usuario cuenta pines, no registros— y dejaría 128 como límite interno de validación. **Barrido pendiente**: comprobar si en las otras dos familias el INFO también hornea algún número que el backend ya sabe.
...
| 25 | — (paquete) | **`samples/GuiImageDemo.bp` se publica y NO puede funcionar desde la instalación.** El sample hace `loadFile("testimg.png")`, y **el `.png` suelto no viaja en el ZIP**: el copiado de `samples/` se lleva sólo los `.bp` (`git ls-files 'samples/*.bp'`), más los árboles de proyecto completos. Así que falla **en placa** (nadie sube el asset) **y en el PC desde el ZIP** (no está el fichero al lado). En el repo funciona porque ahí sí está `samples/testimg.png` — el mismo patrón que el hallazgo 6 con `BpVM.cfg`. Al usuario le sale `cargada = false`, `dims = 0x0` y la pantalla en blanco. **El camino bueno existe y sí viaja entero**: `samples/imageproject/` (`.bpbuild` + `src/` + `resources/testimg.png`, 6.321 B), y su cabecera lo explica — el suelto es la versión de PC, el proyecto la de placa. *Salió porque puse el suelto en la escalera de GUI del P4: error mío al montar la lista, no del producto* | ✅ **CERRADO (6-ago)**: la raíz de `samples/` ya copia **todo lo que está en git**, no sólo los `.bp`, así que `testimg.png` viaja. Y la cabecera del sample explica por qué en placa hace falta `imageproject`: un sample suelto no puede subir recursos **por diseño**, y eso no lo arregla ningún empaquetado |
| 24 | b2 P4 **y c2** | 🔴 **Tras muchas ejecuciones seguidas, la pantalla deja de pintar — y el reset lo cura.** `GuiFontDemo` salió **totalmente negra** (las demás demos dejan fondo blanco). **El modelo estaba PERFECTO**: el volcado enseñaba los 4 labels con `font=12/20/32/48` y sus posiciones, y la consola imprimió todo hasta el final. Tras un **reset de la placa**, el mismo `.mod` sin recompilar **se ve**. Eso descarta el sample, descarta `fontSize` y descarta las fuentes (las cuatro Montserrat que usa están activadas en `include/lv_conf.h`). Lo que queda: **algo se degrada de un RUN al siguiente hasta que deja de pintar**. Lo paró Eduardo antes de que yo siguiera buscando — *«a ver si reseteando se arregla, llevo muchas demos seguidas sin un reset»*—, que es la regla de quitar estado antes de teorizar, y acertó | ✅ **DECIDIDO — NO bloquea, se documenta → V5.** Criterio de Eduardo, y es de dominio: *«si la pantalla deja de pintar porque has salido del programa… esto no es un SO con un prompt, de los programas no se sale nunca. Nosotros lo hacemos porque estamos haciendo pruebas y el programador lo hará porque estará desarrollando pantallas, pero si después de 15 salidas se le cuelga no es ningún problema hacer un reset.»* 🔎 **SEGUNDA VISTA (c2, 5-ago) — y obliga a AFINAR la descripción, no a confirmarla.** Se repitió en la Discovery tras **13 demos encadenadas**, y con reset volvió a verse. Pero el dato que importa es otro: **los DOS avistamientos son el MISMO sample, `GuiFontDemo`**. O sea que lo observado **no** es *«tras N demos se va la pantalla»* sino *«tras N demos, **GuiFontDemo** no pinta»* — y ésas son dos averías distintas con dos arreglos distintos. La alternativa concreta: `GuiFontDemo` es el que **carga el catálogo de fuentes**, el que más memoria de LVGL mueve de toda la escalera; podría no ser estado degradado sino **este sample pidiendo más de lo que queda**. ⚠️ **Y esto es exactamente el error que cometí hoy con el hallazgo 34**: cerrarlo como *«degradación»* con un solo caso, cuando lo que tenía delante era un sample con algo propio. Aquí hay dos casos, pero **del mismo sample**, que no es lo mismo que dos casos independientes. **Experimento que lo separa** (barato, para V5 o para un rato muerto): `reset + UNA demo + GuiFontDemo`. Si con una sola demo previa ya falla, es memoria de este sample; si hacen falta muchas, es acumulación de verdad. Hoy sólo está hecho el `reset + GuiFontDemo` → **se ve**.
...
## 🚪 PUERTA FINAL — prueba 3 de 3: ✅ **PASA**

**`FontLoadDemo` en el micro simulado → OK.** Eduardo: *«he cargado la fuente
primero y he ejecutado FontLoadDemo, todo OK»*.

Detalle que vale la pena anotar: **tuvo que subir la fuente primero**, que es
exactamente lo que dice la cabecera nueva del sample —*«en placa hay que subir el
`.bin` a mano: un sample suelto sólo sube su `.mod`»*—. La documentación y el
comportamiento coinciden, que es más raro de lo que parece.
...
sólo se ven en un despliegue limpio, porque en el de trabajo hay ficheros que
llevan meses ahí y tapan lo que falta.

## Lo que se ha barrido

| | |
|---|---|
| placas | **7**, en **3 familias** (a1 Pico 2 · a2 Metro RP2350B · b1 ESP32-S3 · b2/b3 ESP32-P4 EV y Waveshare · c1 Nucleo-U575 · c2 Discovery U5G9J) + el micro simulado |
| hallazgos registrados | **36** |
| verificados en placa hoy | 12, 29, 34 — y 20/36/37 en el emulador |
| imágenes de firmware | **5**, todas reconstruidas del mismo árbol y regeneradas en `dist/firmware` |

## Los dos bugs de verdad que salieron, y cómo salieron

1. **El firmware STM32 se publicaba sin optimizar** (`-O0`, intérprete incluido).
   No salió por sospecha: salió porque **un número no cuadraba con el silicio**
   —un Cortex-M33 con flash interna peor por ciclo que un Xtensa—. `-Os`: **3,65×
   más rápido**, imagen **−40 %**.
2. **El azúcar `parallel/case` truncaba el handle del thread a 4 bytes.** Sólo se
   manifiesta cuando ya se han reciclado slots de la tabla de handles; hasta
   entonces las generaciones valen 0 y el truncamiento funciona por casualidad.
   Lo cazó una receta de Eduardo —`reset + MemInfo + sugar`— que convirtió una
   lotería en un experimento reproducible.

## El patrón que se repitió CINCO veces

Un **artefacto generado que se queda atrás y nadie avisa**: el `Hello.mod` v5
embebido, el `Bench.mdn` ABI 1, los 24 `.mod` intrusos de `bpstdlib/`, las cinco
imágenes rancias de `dist/firmware`, y la fuente que **nunca estuvo en git**.
Ninguno era un error de programación; los cinco eran **la falta de un guardián**.
De ahí salen los tres que se han puesto en esta campaña —el gate de MOD6, el
guardián de `PUBLICAR_DIRS`, y el de fin de RUN (#339)— y el que queda propuesto
para V5: **verificar que los recursos que un sample nombra están en git**.

Y su hermano: **un fallo mudo tapa a otro**. El simulador sin LVGL escondía que
`ChartDemo` no viajaba; `loadFont` sin señal de error escondió durante una
versión entera que faltaba la fuente. Cuando el síntoma apunta al sitio
equivocado, el coste se dobla.

## Lo que NO se cierra, y dónde ha ido

Nada de esto bloquea la publicación; todo tiene destino escrito:

- **#378** — que cada micro DIGA lo que tiene (capa HAL BP de capacidades).
  Decisión de Eduardo: los números de periféricos son informativos.
- **#379** — el wire se desincroniza tras el Stop, y **sólo en unas placas**. Con
  el discriminador ya en la mano.
- **11** (el preinstalado no se reemplaza) · **19** (`variant` en el S3, con el
  reencuadre de Eduardo) · **21a** (no hay driver WS2812 fuera del RP2350) ·
  **`loadFont` sin señal de fallo** · **#373** (rendimiento del GUI).

## Lo que queda antes de publicar — **no es H13, es la fase de publicación**

1. **Decisión de Eduardo**: `docs/TERCEROS.md` con las licencias que ya
   redistribuimos (LVGL, littlefs, SDL2, FreeRTOS, Montserrat).
2. **#375** — la guía rápida de BP, 1 página HTML en inglés, **fuera** del ZIP.
3. Los pasos de `docs/PUBLICAR.md`: release, Pages y los binarios.
