...
| 15 | — (paquete) | **72 artefactos generados, caducados, viajan en el ZIP.** `bpstdlib/` —que `BpVM.cfg` declara como `stdlibDir`— tiene 26 módulos legítimos (v6) y **24 intrusos, todos v5**: 23 demos de Gui y `L2Lib`, ninguno con `.bp` que lo respalde. Y `samples/` lleva **24 `.mod` (v5) + 24 `.dbg`** trackeados. Salió al probar `l2app`: el `L2Lib.mod` rancio de `bpstdlib` pisó al recién compilado. Nadie los usa como entrada —comprobado uno a uno—, pero quien abra la librería estándar se encuentra `GuiTableDemo.mod` dentro | 🟡 **EN EL LOTE, decidido por Eduardo: se borran al terminar la campaña**, no ahora. Criterio objetivo de qué sobra: en `bpstdlib/`, `.mod` sin `.bp` al lado; en `samples/`, todo `.mod`/`.dbg`. Va con regla de `.gitignore` para que no vuelvan. Mismo hallazgo que el 2 y el 5: generados que se commitean y se pudren |
