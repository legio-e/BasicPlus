...
| 37 | — (paquete) | 🔴 **El MICRO SIMULADO que se publica NO PUEDE PINTAR: `bpvm-sim.exe` está compilado SIN LVGL.** Salió porque Eduardo quiso comprobar el hallazgo 36 en el emulador del PC y vio que *«el simulador va, ejecuta los programas incluido los gráficos, pero la pantalla no se ve»*. Su diagnóstico —*«o es un problema con la parte gráfica o falta configurar algo»*— era el correcto, y la respuesta es **el build, no la configuración**: sus preferencias están bien (`simNoScreen:0`, `simScreenW/H:480x800`, `~/.bpide-prefs`) y el `--screen=WxH` se acepta… y se ignora. **Comprobado en el binario**: `bpvm-sim.exe` tiene **CERO referencias a `SDL2.dll` y CERO símbolos de LVGL**. Sin `BPVM_LVGL`, `gui.c` es sólo el modelo —crea los widgets, contesta al volcado, no pinta un píxel—, así que las demos gráficas *parecen* correr. **Y no es la copia local de Eduardo: es el binario del PAQUETE.** `montar-zip.sh:60` copia ese mismo `.exe` a `bin/`, y el de `C:	emp\BasicPlus-4.0-winin\` es **byte a byte el mismo**. 🃏 **La guinda**: el ZIP **sí lleva `SDL2.dll`** al lado — una DLL que el ejecutable **nunca carga**. Todo parece en su sitio |
...
| 36 | — (paquete) | 🔴 **`FontLoadDemo` NO PUEDE FUNCIONAR tal como se publica: le falta el fichero de la fuente.** Preguntó Eduardo —*«¿`FontLoadDemo` era un proyecto? Hacía falta la fuente»*— y la respuesta es peor que un despiste de memoria: **es un sample suelto de `samples/`** (no un proyecto, así que **no tiene `resources/`** que le lleve nada a la placa) y pide `Gui.loadFont("montserrat_26_bold.bin")` (`FontLoadDemo.bp:26`), un `.bin` de LVGL que **el código lee de `/app` en placa**. **Ese fichero NO EXISTE**: buscado en el repo, en el ZIP, en la instalación de `C:	emp` y en **todo el historial de git** — cero. Luego, recién instalado, `loadFont` falla siempre |
