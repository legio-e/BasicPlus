...
## SQLite en el micro — arquitectura, hallazgos y plan de pruebas

**Charla del 6-ago-2026 (tarde), tras publicar V4.** Diseño acordado, **cero código**.

### La arquitectura de Eduardo

```
AppBP  -->  Pack [ SQLite.mod --> [ bridge.c + SQLite ] ]  -->  BIOS
```

- `AppBP → Pack[SQLite.mod]` **ya está probado y funciona**.
- `bridge.c` es un puente **doble**: entre `SQLite.mod` y SQLite, y entre SQLite y el
  resto del sistema (memoria, FS…). Debe ser **reubicable**: no sabemos dónde caerá el
  pack.
- La **BIOS** es una tabla de servicios, pequeña, porque va en **todas** las imágenes —
  la decisión de usar BD es del usuario, no nuestra.

### Hallazgo 1 — la BIOS ya existe: `aot_helpers_v2`

No hay que construirla. Es una tabla de punteros a función, el código AOT la usa como
`vm->aot_helpers->func(...)`, y **ya está versionada** (v1→v2, cuando las refs pasaron a
8B). O sea que ya pagamos la lección de que esa tabla es un **ABI**.

Y resuelve el «posición fija o mecanismo de localización» sin ninguna de las dos: el
código cargado **no la busca, se la pasan** por el puntero de la VM. Quien carga el pack
ya sabe dónde está la BIOS. Una dirección fija sería frágil entre las 3 familias y entre
versiones de firmware.

Sobre el tamaño: la tabla son decenas de bytes y **lo que apunta ya está en la imagen**
(la VM usa ese FS y esa memoria igual). La BIOS **re-expone, no añade**. El coste sólo
crece si SQLite pide servicios que hoy no tenemos — y ahí está el ojo: si nuestro FS no
sabe leer por posición o abrir varios descriptores del mismo fichero, **eso sí se le
añade a todas las imágenes**.

### Hallazgo 2 — nuestro cargador NO reubica. Éste es EL riesgo.

`mdn_loader.c` es **zero-copy**: calcula `thunk_addr = code_base + sym.thunk_offset` y
registra el puntero. **No hay tabla de relocalizaciones, ni parche de direcciones, ni
GOT.** El código se queda donde cayó.

Funciona porque **nuestro emisor AOT genera bajo una disciplina**: nunca emite una
dirección absoluta y todo lo del runtime lo pide por el puntero de helpers. Es reubicable
*por construcción*, no porque alguien lo reubique.

**SQLite no está escrito bajo esa disciplina.** Tiene lo de cualquier C real:

| | riesgo |
|---|---|
| literales en `.rodata` | alcanzables por PC-relativo, probablemente bien |
| **tablas estáticas de punteros a función** | direcciones absolutas → **hay que reubicar** |
| **estado estático mutable (`.data`/`.bss`)** | tiene que vivir en RAM, y el código debe encontrarlo |

Las dos últimas rompen el esquema actual. Conclusión: **el camino `.mdn` tal como está no
lo lleva**. Haría falta un cargador que reubique, o ROPI/RWPI (existe en el compilador de
ARM; en `arm-none-eabi-gcc` **hay que comprobarlo**), o meter SQLite en la imagen — que es
justo lo que no se quiere.

**Por eso esta prueba va PRIMERA**: es lo único que puede tirar abajo el diseño entero.

### Hallazgo 3 — el PC no sirve para probar la reubicación

Y daría un **verde que no significa nada**:

- en x86-64 casi todo es relativo al PC y `-fPIC` es casi gratis; lo que ahí pasa puede
  reventar en Thumb-2 o RISC-V;
- la tentación de probar con `dlopen` es peor todavía: eso usa **el enlazador dinámico del
  sistema**, que sí reubica. Estaríamos probando el cargador de Linux, no el nuestro;
- nuestro cargador lleva el bit Thumb dentro (`| 1u`), que en x86 no tiene equivalente.

Es el patrón de [[instrumento-mudo-dudar-de-el]]. **La reubicación es de placa, sin
atajo, y en las dos arquitecturas.**

Los **puentes** en cambio sí se prueban en PC, y no como simulación: el host tiene su
propio camino AOT (`test-callbp` existe), así que se ejercita el mismo registro de
símbolos, sólo que sin el bit de modo.

### El puente: qué cruza y quién lo sujeta

**Descartado: que la frontera copie el resultado completo.** *(corrección de Eduardo)* Un
`SELECT` no devuelve un valor, devuelve un conjunto; materializarlo en el puente para
luego pasarlo duplica la memoria y en un micro eso es el final.

**Lo que sí es inevitable**: en el sentido C→BP **siempre hay una copia**. Una cadena BP es
un objeto del heap de la VM (un handle); no se puede envolver memoria ajena en un objeto
BP. Lo que hay que evitar es la **segunda** copia: fila a fila, valor a valor, de la
memoria de SQLite directamente al objeto BP recién creado. El puente nunca sujeta más de
una fila.

**Array acotado de referencias en el puente** *(idea de Eduardo)* — el conjunto de raíces
del GC, explícito. El recolector recorre N ranuras. Sin shadow stack, sin barrido
conservador, y **sin pedirle cooperación a SQLite**, que era el problema de fondo de
#302 paso 3. Matices:

- **es más pequeño de lo que parece**: no guarda lo que SQLite sujeta (memoria suya), sólo
  **objetos BP que el puente tiene en la mano**. No es «por conexión», es «por llamada en
  vuelo» → del orden de *hilos × unas pocas*;
- **la dirección que de verdad lo necesita es la de vuelta**: cuando el puente crea el
  objeto BP de la columna 1, **todavía no lo sujeta nadie**; si al construir la 2 entra un
  GC, la 1 se muere. Es exactamente la forma de **#350** (agujero de GC en builtins de
  cadenas, 3-6 de cada 3000);
- **por hilo, no global** — cada hilo ya tiene su contexto en la VM; un array compartido
  necesitaría cerrojo en el camino caliente;
- **acotado = puede fallar en voz alta**. Mejor un límite con mensaje claro que una
  estructura que crece y un día se come el heap en silencio.

### Conexiones: el límite

**Criterio de Eduardo:** la VM ejecuta **un** programa; puede haber una conexión abierta y
necesitar otra consulta dentro, y lo mismo en otro hilo → **4**. Al quinto, error, y que el
programador se replantee el diseño: *«estamos en un micro; si quiere gastar toneladas de
memoria, se ha equivocado de sitio»*.

Dos apuntes:

- **el 4 es un presupuesto de RAM disfrazado**: lo que cuesta una conexión es su caché de
  páginas. Escribirlo como *arena ÷ coste-por-conexión* aunque hoy dé 4, igual que el
  reparto 25/75 de la Pico o los 160 KB del S3. El día que se mueva, es un número.
- el error debe ser **excepción BP atrapable** —como el OOM desde V4— y el mensaje debe
  decir **qué hacer**, no sólo que no se puede.

**⚠️ Pregunta ABIERTA que decide la API** — Claude afirmó que una conexión admite varias
`sqlite3_stmt` a la vez y que el caso «consulta dentro de consulta» se resuelve con
*sentencias*, no con conexiones; Eduardo recuerda haberse topado con lo contrario y haber
tenido que reescribir código. Sospecha de Claude: era **JDBC** (`java.sql.Statement`
permite **un solo `ResultSet` abierto** por objeto), no SQLite. Ninguno de los dos lo da
por seguro. **Se decide midiendo, no recordando** → prueba 2-bis.

Restricción que sí existe en C y podría ser lo que mordió: **escribir en una tabla con un
`SELECT` en vuelo sobre esa misma tabla** deja indefinido lo que el `SELECT` ve, y según
versión/modo puede dar `SQLITE_LOCKED`.

### El FS: flash interna vs SD

**No hay ningún soporte de SD hoy** (los aciertos de `grep` son cabeceras del HAL de ST y
el `sdkconfig` de Espressif). Es *bring-up* de hardware nuevo → la prueba del FS no espera
una decisión, espera trabajo de placa.

La pregunta «¿vale probar con flash interna?» **se parte en dos**:

- **la mitad de forma SÍ se transfiere**: varios descriptores del mismo fichero con
  posición independiente, leídos/escritos desde dos hilos. Eso es API, no medio.
- **la mitad física NO**:

| | flash interna | SD |
|---|---|---|
| escritura | borrar sector entero (4 KB) | bloques de 512 B, la FTL gestiona el borrado |
| mientras escribe | en RP2xxx **no se puede ejecutar desde flash** | otro bus, la CPU libre |
| coste | ms con todo parado | ms sin bloquear |
| corte de luz | corrompe lo que escribías | la FTL puede corromper **otros** bloques |
| FS | littlefs | FAT |

La segunda fila importa mucho aquí: si el código de SQLite corre XIP desde el pack en
flash interna **y** escribe en flash interna, estás borrando el bus por el que te
ejecutas. Con SD no existe ese problema. Y la última fila da un resultado **optimista** en
flash: no hay FTL que se lleve bloques vecinos.

### Decisiones ya tomadas (Eduardo)

- SQLite irá probablemente **en SD** — la flash se queda pequeña.
- **Tiene que usar el FS**, no ir por libre. *(Queda sobre la mesa un punto medio: otra
  cintura bajo la misma fachada, con una región cruda de un solo fichero — no es ir por
  libre, es lo que ya hicimos con los packs.)*
- Empezar por **journal**, no WAL: más simple y más robusto; WAL sólo si se mide que hace
  falta.
- **Nada de FS todavía** en las pruebas: primero reubicable, luego puente 1, luego puente 2.
- 🎯 **V5 arranca CENTRADA en esto.** *«Prefiero que al principio de la V5 estemos
  centrados en esto y que el resto de cosas no nos distraiga. Si lo conseguimos, ya
  pasaremos a estudiar el resto.»* Mismo criterio que
  [[focus-un-kit-batch-cross-family]], aplicado a features. Lo que aparezca por el camino
  **se anota y se sigue**.
...
|---|---|---|---|
| 0 | `.c` de 20 líneas con **literal + tabla estática de punteros + estático mutable**, por el camino `.mdn` | **placa ARM** | si la arquitectura es viable tal cual |
| 1 | lo mismo | **placa RISC-V** | si hace falta un camino por arquitectura |
| 2 | puente BP → C ajeno, **con un GC en medio** | PC | forma del puente + si #302 paso 3 nos alcanza |
| 2-bis | 1 conexión, 2 `SELECT` alternando `step()`; + `INSERT` en otra tabla; + en la misma | PC | **`Connection`+`Statement` o conexiones**: son dos APIs distintas |
| 3 | puente C → `aot_helpers`, **comprobando la versión de la tabla** | PC | forma de la BIOS |
| — | *aquí ya se puede decidir la arquitectura* | | |
| 4 | SQLite real con `SQLITE_OS_OTHER=1` + VFS mínimo sobre `stdio` | PC | qué hay que rellenar de verdad |
| 5 | RAM con `MEMSYS5` (alocador propio sobre buffer fijo) y caché pequeña | PC | en qué placas cabe |
| 6 | varios descriptores posicionales desde dos hilos | PC o placa | si el FS aguanta varias conexiones |
| 7 | *bring-up de SD* → rendimiento y **corte de alimentación ×100** | placa + SD | la robustez de verdad |

Notas de compilación para cuando toque: `SQLITE_OS_OTHER=1` (sin nada de SO),
`SQLITE_TEMP_STORE=3` (temporales en RAM, evita ficheros temporales en el VFS),
`SQLITE_THREADSAFE=1` para empezar (serializado: más lento, mucho más difícil de romper).

**Estado: sin implementar. Mañana, la 0.**
