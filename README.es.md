...
Raspberry Pi Pico 2, un ESP32 (S3, C3, C6 o P4) o un STM32 — sin recompilar, sin
`#ifdef`, sin sorpresas.
...
**V6 — orden en los micros** (septiembre 2026). La V1 demostró la idea; la V2 la
endureció y la amplió; la V3 le puso cara; la V4 le arregló los cimientos; la V5
le dio dónde guardar cosas; **la V6 pone orden en el código de los
microcontroladores**: lo que hacía lo mismo en cada micro ahora es común —algo
más del 90 % del firmware, medido sobre lo que enlaza cada imagen; un 7-8 % es de
cada familia (su HAL y la capa HAL de BasicPlus) y menos de un 1 % de cada placa
(el arranque y sus particularidades)—, así que añadir un micro de una familia ya
soportada es relativamente sencillo. Entran dos micros nuevos, el **ESP32-C3** y
el **ESP32-C6** (con la primera pantalla por SPI); la interfaz gráfica corre en
